...
```{r}
model <- model %>%
  replace(Equation("KA", "THETA_KA*exp(ETA_KA + IOV_KA)"))
```
...
```{r}
ds_no_iov <- ds_iov %>%
  replace(IOV("IOV_KA", 0))
```

We can now run the model with IOV on `KA` and without.

```{r lag_time_model , fig.align='center', fig.height=4, fig.width=8, message=F}
results_iov <- simulate(model = model, dataset = ds_iov, seed = 1)
results_no_iov <- simulate(model = model, dataset = ds_no_iov, seed = 1)
gridExtra::grid.arrange(
  shaded_plot(results_iov, "CONC"),
  shaded_plot(results_no_iov, "CONC"),
  nrow = 1
)
```
