...
setClass(
  ...
  contains = "pmx_list",
  prototype = prototype(
    type = "treatment_entry",
    iovs = new("treatment_iovs"),
    occasions = new("occasions"),
    dose_adaptations = new("dose_adaptations")
  )
)
...
getAdminString <- function(object, type) {
  ...
  table <- admins %>%
    ...
    dplyr::mutate(
      AMT_STRING = ifelse(duplicated(.data$AMT), sprintf("%s", .data$TIME), sprintf("%s (%s)", .data$TIME, .data$AMT))
    )
...
setMethod(
  "update_amount",
  signature = c("treatment", "numeric", "character"),
...
setMethod(
  "update_repeat",
  signature = c("treatment", "repeated_schedule", "character"),
