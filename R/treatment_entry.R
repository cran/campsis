...
check_treatment_entry <- function(object) {
  return(c(
    expect_one_for_all(object, c("amount", "dose_number", "ref")),
...
Bolus <- function(
  ...
  wrapper <- new(
    ...
    f = to_explicit_distribution_list(f, cmtNo = cmtNo),
    lag = to_explicit_distribution_list(lag, cmtNo = cmtNo),
    ii = iiAddl$ii,
    addl = iiAddl$addl,
    ref = process_ref_arg(ref),
    rep = process_repeat_arg(rep, iiAddl)
  )
  if (wrap) {
    return(wrapper)
  } else {
...
validate_infusion <- function(object) {
  return(c(expect_one_or_more(object, "time"), expect_one_or_more(object, c("duration", "rate"))))
}
...
Infusion <- function(
  ...
  wrapper <- new(
    ...
    f = to_explicit_distribution_list(f, cmtNo = cmtNo),
    lag = to_explicit_distribution_list(lag, cmtNo = cmtNo),
    duration = to_explicit_distribution_list(duration, cmtNo = cmtNo),
    rate = to_explicit_distribution_list(rate, cmtNo = cmtNo),
    ii = iiAddl$ii,
    addl = iiAddl$addl,
    ref = process_ref_arg(ref),
    rep = process_repeat_arg(rep, iiAddl)
...
check_ii_and_addl <- function(time, ii, addl) {
  if (is.null(ii) && is.null(addl)) {
    ...
    assertthat::assert_that(
      is.numeric(addl) && length(addl) == 1 && addl %% 1 == 0 && !is.na(addl),
      msg = "addl must be a single integer value"
    )
...
setMethod("sample", signature = c("bolus", "integer"), definition = function(object, n, ...) {
  ...
  retValue <- tibble::tibble(
    ...
    TIME = object@time + lag,
    EVID = as.integer(1),
    MDV = as.integer(1),
    AMT = object@amount * f,
    CMT = rep(depotCmt, length(ids)),
    RATE = as.numeric(0),
    DOSENO = object@dose_number,
    INFUSION_TYPE = as.integer(0),
...
setMethod("sample", signature = c("infusion", "integer"), definition = function(object, n, ...) {
  ...
  f <- sample_trt_distributions(distributions = object@f, n = n, default = 1, compartmentNo = compartmentNo)
  lag <- sample_trt_distributions(distributions = object@lag, n = n, default = 0, compartmentNo = compartmentNo)
  rate <- sample_trt_distributions(
    distributions = object@rate,
    n = n,
    default = as.numeric(NA),
    compartmentNo = compartmentNo
  )
  duration <- sample_trt_distributions(
    distributions = object@duration,
    n = n,
    default = as.numeric(NA),
    compartmentNo = compartmentNo
  )
...
setMethod(
  "update_amount",
  signature = c("infusion", "numeric", "character"),
...
setMethod(
  "update_amount",
  signature = c("bolus_wrapper", "numeric", "character"),
  definition = function(object, amount, ref) {
    return(update_amountDelegate(object, amount, ref))
  }
)
...
setMethod(
  "update_amount",
  signature = c("infusion_wrapper", "numeric", "character"),
  definition = function(object, amount, ref) {
    return(update_amountDelegate(object, amount, ref))
  }
)
...
setMethod(
  "update_ii",
  signature = c("infusion_wrapper", "numeric", "character"),
  definition = function(object, ii, ref) {
    return(update_iiDelegate(object, ii, ref))
  }
)
...
setMethod(
  "update_addl",
  signature = c("bolus_wrapper", "integer", "character"),
  definition = function(object, addl, ref) {
...
setMethod(
  "update_addl",
  signature = c("infusion_wrapper", "integer", "character"),
  definition = function(object, addl, ref) {
    return(update_addl_delegate(object, addl, ref))
  }
)
...
setMethod(
  "update_repeat",
  signature = c("bolus_wrapper", "repeated_schedule", "character"),
...
setMethod(
  "update_repeat",
  signature = c("infusion_wrapper", "repeated_schedule", "character"),
  definition = function(object, rep, ref) {
    return(update_repeat_delegate(object, rep, ref))
  }
)
...
setMethod(
  "update_repeat",
  signature = c("bolus", "repeated_schedule", "character"),
  definition = function(object, rep, ref) {
    return(object) # Do nothing
  }
)
...
setMethod(
  "update_repeat",
  signature = c("infusion", "repeated_schedule", "character"),
  definition = function(object, rep, ref) {
    return(object) # Do nothing
  }
)
