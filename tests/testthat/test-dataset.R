...
test_that("Set subjects works as expected", {
  ...
  expect_error(
    Dataset() %>% set_subjects(c(10, 20)),
    regexp = "x must be the same length as the number of arms in dataset"
  )
  dataset <- Dataset() %>%
...
test_that("Export function covariates work well (N=3)", {
  ...
  subTable <- table %>%
    ...
    dplyr::distinct() %>%
    dplyr::mutate(WT = round(WT), HT = round(HT))
...
test_that("Export boostrap covariates work well (N=8)", {
  ...
  expect_equal(
    ...
    tibble::tibble(
      ...
      WT = c(65, 75, 65, 70, 65, 75, 75, 70),
      HT = c(180, 185, 185, 175, 175, 175, 180, 180)
    )
  )
})
...
test_that("Compartment argument both accepts a character vector with compartment names", {
  ...
  expect_true(any(
    "-> Adm. times (bolus into CMT=DEPOT1,DEPOT2,DEPOT3): 0 (100),24" %in%
  ...
  table <- dataset %>%
    export(dest = "rxode2")
  expect_equal(
    table %>% dplyr::select(c("TIME", "AMT", "CMT")),
    tibble::tibble(
      TIME = c(0, 0, 0, 24, 24, 24),
  ...
  expect_true(any(
    "-> Adm. times (infusion into CMT=SC1,SC2): 0 (100),24" %in%
      capture.output(show(dataset))
  ))
  ...
  table <- dataset %>%
    export(dest = "rxode2")
  expect_equal(
    table %>% dplyr::select(c("TIME", "AMT", "CMT", "RATE")),
    tibble::tibble(
      TIME = c(0, 0, 24, 24),
  ...
  expect_error(
    dataset %>% export(dest = "rxode2", model = model),
    regexp = "Unknown compartment name\\(s\\): DEPOT2, DEPOT3"
  )
})
...
test_that("Compartment properties can be vectorised", {
  ...
  infusion <- Infusion(
    time = 0,
    amount = 100,
    compartment = c("CENTRAL1", "CENTRAL2"),
    ii = 24,
    addl = 2,
    duration = list(dist1, dist2),
    ref = "Admin1"
  )
  bolus <- Bolus(
    time = 0,
    amount = 100,
    compartment = c("DEPOT1", "DEPOT2"),
    ii = 24,
    addl = 2,
    f = c(0.7, 0.3),
    ref = "Admin1"
...
test_that("Method 'update_repeat' works as expected", {
  bolus <- Bolus(
    ...
    ii = 24,
    addl = 2,
    rep = CyclicSchedule(24 * 7, 1),
    ref = "Admin1"
  )
  infusion <- Infusion(
    time = 0,
    amount = 100,
    compartment = c("DEPOT3"),
    ii = 24,
    addl = 2,
    rep = CyclicSchedule(24 * 7, 1),
    ref = "Admin1"
...
test_that("Assertion in method 'get_compartment_mapping' should properly work", {
  ...
  expect_error(
    dataset %>% export(dest = "rxode2", model = model, seed = 1),
