...
test_that("Hardware settings work as expected", {
  ...
  expect_error(
    Settings(Hardware(
      ...
      slice_parallel = TRUE,
      slice_size = 100,
      dataset_parallel = TRUE,
      dataset_slice_size = 250
    )),
    regexp = "Parallelization can only be applied at one level"
  )
})
...
test_that("Solver settings work as expected", {
  ...
  expect_true(
    "Solver: atol=1.0e-12, rtol=1.0e-12, hmax=NA, maxsteps=70000, method=liblsoda" %in% capture.output(show(settings))
  )
})
...
test_that("Replication/parallelisation settings work as expected", {
  ...
  settings <- Settings(
    Hardware(cpu = 6, replicate_parallel = T),
    AutoReplicationSettings(wishart = TRUE, odf = 50, sdf = 1000)
  )
...
test_that("Unknown settings shouldn't be accepted", {
  ...
  expect_error(
    Settings(Hardware(cpu = 4, replicate_parallel = TRUE), A = c(1, 2, 3)),
    regexp = "Unknown argument detected"
  )
})
