...
test_that("Simulate scenarios - make few changes on dataset (RxODE/mrgsolve)", {
  ...
  results1 <- model %>% simulate(dataset=dataset, dest="RxODE", scenarios=scenarios, seed=seed)
...
test_that("Simulate scenarios - make few changes on model (RxODE/mrgsolve)", {
  model <- model_library$advan4_trans4
  ...
  results1 <- model %>% simulate(dataset=ds, dest="RxODE", scenarios=scenarios, seed=seed)
